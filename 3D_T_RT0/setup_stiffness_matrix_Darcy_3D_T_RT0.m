function [Kloc,signs] = setup_stiffness_matrix_Darcy_3D_T_RT0(coeffs,nelem,elems2nodes,elems2faces,faces2nodes,nodes2coord)
% 
% 
% ----------------------------------------------------------------------
...
nelf = 4; % = 3 for 2D triangle, 4 for 3D tetrahedron
% number of dofs per element 
...
for e = 1:nelem
   ...
   J = [xcoord(2)-xcoord(1) ycoord(2)-ycoord(1) zcoord(2)-zcoord(1);
        xcoord(3)-xcoord(1) ycoord(3)-ycoord(1) zcoord(3)-zcoord(1);
        xcoord(4)-xcoord(1) ycoord(4)-ycoord(1) zcoord(4)-zcoord(1)];
%    invJ = inv(J);
%    JJ(:,:,e) = J;
...
%    matmtx = coeffs(e);
   ...
   for ip = 1:size(point2,1)
      x = point2(ip,1); 
      y = point2(ip,2);
      z = point2(ip,3);
      wip = weight2(ip);
      if abs(1-x-y-z) > eps, error(''), end
      
      % values of shape functions and derivatives at quadrature point
      [sqhape,divsqhape] = feisotetrahedr3D4n_RT0(x,y,z);
...
%       [dNdx,dNdy,dNdz] = federiv3D(dNvdksi,dNvdeta,dNvdzeta,invJ);
      ...
      sqhape1 = spdiags(signs(:,e),0,nelf,nelf) * ( 1/(6*detJ) * (sqhape*J) * spdiags(coeffs(:,e),0,dim,dim) );
      sqhape2 = spdiags(signs(:,e),0,nelf,nelf) * 1/(6*detJ) * (sqhape*J);
