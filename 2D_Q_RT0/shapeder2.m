function [dshape] = shapeder2(point,etype)
...
%       l2 = 1-l1;
%       switch etype
%          case {'P0'} % Constant shape function
%             dshape = zeros(1,1,nop);
%          case {'P1'} % Linear shape functions
%             dshape = [1 -1];
...
%       end
   case {2} % 2-D elements
      ...
      l3 = 1 - l1 - l2;
      switch etype
         case {'P0','Q0'} % Constant shape function
         ...
         case {'Q1'} % by Bedrich Sousedik, 2/16/2016
            % bi-linear (4 node) quadrilateral
            % (x,y,z) is a point on local coordinate
            % 4 basis nodal functions
            x = point(1,:);
            y = point(2,:);
     
            s = [0 0; % ATTENTION ON ORDER OF POINTS IN S
                 1 0;
                 1 1;
                 0 1] * 2 - 1;
            dshape = zeros(2,4,nop);   % 3: 3D x,y,z; 8: basis of 8 functions; nop: number of points
            for i = 1:4
               dshape(1,i,:) = 1/4 * s(i,1) .* (1+s(i,2)*y);  % d(phi_i)/dx
               dshape(2,i,:) = 1/4 * (1+s(i,1)*x) .* s(i,2);  % d(phi_i)/dy
            end  
          case {'Q2'}
            % bi-linear (4 node) quadrilateral
            % (x,y,z) is a point on local coordinate
