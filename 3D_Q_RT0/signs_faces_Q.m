...
% numbering as in Anjam-2015-FMA:
% n1 = squeeze(amsv(B_K_invT, [ 0  0 -1]))';
% n2 = squeeze(amsv(B_K_invT, [ 0 -1  0]))';
% n3 = squeeze(amsv(B_K_invT, [-1  0  0]))';
% n4 = squeeze(amsv(B_K_invT, [ 1  1  1]))';
% me and Matlab numbering:
n1 = squeeze(amsv(B_K_invT, [ 0  0  1]))';
n2 = squeeze(amsv(B_K_invT, [1  0  0]))';
n3 = squeeze(amsv(B_K_invT, [ 0 1  0]))';
n4 = squeeze(amsv(B_K_invT, [ 0  0 -1]))';
n5 = squeeze(amsv(B_K_invT, [ -1  0 0]))';
n6 = squeeze(amsv(B_K_invT, [ 0  -1 0]))';
...
tmp(:,5) = sum(normals(elems2faces(:,5),:) .* n5, 2);
tmp(:,6) = sum(normals(elems2faces(:,6),:) .* n6, 2);
...
if any(isnan(signs))
   error('Some sign of a face is NaN.')
end   
